...
prepare_initramfs_dracut(){
    ...
    frostools-chroot "$mnt" dracut -fqM /boot/initramfs.img "$kver"
...
write_mkinitcpio_conf() {
    ...
    local conf="$1/etc/mkinitcpio-frost.conf"
    printf "%s\n" 'MODULES=(loop dm-snapshot)' > $conf
    printf "%s\n" 'COMPRESSION="xz"' >> $conf
    if [[ "${PROFILE}" == 'base' ]];then
        printf "%s\n" 'HOOKS=(base udev frost_shutdown frost frost_loop_mnt
                            frost_pxe_common frost_pxe_http frost_pxe_nbd frost_pxe_nfs
                            frost_kms modconf block filesystems keyboard keymap)' >> $conf
    else
        printf "%s\n" 'HOOKS=(base udev frost_shutdown frost frost_loop_mnt
                            frost_kms modconf block filesystems keyboard keymap)' >> $conf
    fi
...
prepare_initcpio(){
    ...
    local dest="$1"
    cp /etc/initcpio/hooks/frost* $dest/etc/initcpio/hooks
    cp /etc/initcpio/install/frost* $dest/etc/initcpio/install
    cp /etc/initcpio/frost_shutdown $dest/etc/initcpio
}
...
prepare_initramfs(){
    ...
    FROST_GNUPG_FD=${GPG_KEY:+17} frostools-chroot "$mnt" \
        /usr/bin/mkinitcpio -k ${_kernel} \
        -c /etc/mkinitcpio-frost.conf \
...
prepare_grub(){
    ...
    cp -r ${theme}/themes/frost ${grub}/themes
    ...
    mkfs.fat -n FROST_EFI "${efi_img}" &>/dev/null
    prepare_dir "${mnt}"
