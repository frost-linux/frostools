...
write_postcfg(){
    ...
    for k in archlinux artix frost;do
...
#     else
    yaml+=$(write_yaml_seq_map 2 'source' '"/run/frost/bootmnt/LiveOS/rootfs.img"')
...
configure_chroot(){
    ...
    [[ ! -d "$fs/etc/frostools" ]] && mkdir -p "$fs/etc/frostools"
    msg2 "Writing live.conf"
    write_live_session_conf > "$fs/etc/frostools/live.conf"
